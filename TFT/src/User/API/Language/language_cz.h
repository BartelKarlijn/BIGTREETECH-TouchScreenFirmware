#ifndef _LANGUAGE_CZ_H_
#define _LANGUAGE_CZ_H_

    #define CZ_LANGUAGE               "Čeština"
    #define CZ_HEAT                   "Teplota"
    #define CZ_MOVE                   "Posunout"
    #define CZ_HOME                   "Domů"
    #define CZ_DISABLE_STEPPERS       "Steppers"
    #define CZ_PRINT                  "Tisknout"
    #define CZ_EXTRUDE                "Filament"
    #define CZ_FAN                    "Ofuk"
    #define CZ_SETTINGS               "Nastavení"
    #define CZ_SCREEN_SETTINGS        "Vzhled"
    #define CZ_MACHINE_SETTINGS       "Tiskárna"
    #define CZ_FEATURE_SETTINGS       "Vlastnosti"
    #define CZ_SILENT_ON              "Tichý"
    #define CZ_SILENT_OFF             "Pípání"
    #define CZ_SHUT_DOWN              "Vypnout"
    #define CZ_RGB_SETTINGS           "Barva LED"
    #define CZ_RGB_OFF                "LED vyp."
    #define CZ_TERMINAL               "Terminal"
    #define CZ_CUSTOM                 "Vlastní"
    #define CZ_LEVELING               "Srovnat"
    #define CZ_POINT_1                "Bod 1"
    #define CZ_POINT_2                "Bod 2"
    #define CZ_POINT_3                "Bod 3"
    #define CZ_POINT_4                "Bod 4"
    #define CZ_POINT_5                "Bod 5"
    #define CZ_ABL                    "Vyrovnat"
    #define CZ_ABL_COMPLETE           "Vyrovnání Komplet"
    #define CZ_ABL_SMART_FILL         "Chybějící body sondy\nbyly vyplněny inteligentně.\n \nPozn: Nezapomeň uložit!"
    #define CZ_ABL_SETTINGS           "Bilineární vyrovnání"
    #define CZ_ABL_SETTINGS_UBL       "UBL vyrovnání"
    #define CZ_ABL_SETTINGS_UBL_SAVE  "Uložit do slotu"
    #define CZ_ABL_SETTINGS_UBL_LOAD  "Nahrát ze slotu"
    #define CZ_ABL_SLOT0              "Slot 0"
    #define CZ_ABL_SLOT1              "Slot 1"
    #define CZ_ABL_SLOT2              "Slot 2"
    #define CZ_ABL_SLOT3              "Slot 3"
    #define CZ_ABL_SLOT_EEPROM        "Zapamatuj si slot\nrestart? (Uložit EEPROM)"
    #define CZ_ABL_Z                  "Z zeslabení"
    #define CZ_ABL_ENABLE             "ABL: on"
    #define CZ_ABL_DISABLE            "ABL: off"
    #define CZ_BLTOUCH                "BLTouch"
    #define CZ_BLTOUCH_TEST           "Test"
    #define CZ_BLTOUCH_DEPLOY         "Vysunout"
    #define CZ_BLTOUCH_STOW           "Zasunout"
    #define CZ_BLTOUCH_REPEAT         "Přesnost"
    #define CZ_Z_OFFSET               "Z Offset"
    #define CZ_PROBE_OFFSET           "Ofset sondy"
    #define CZ_CONFIRMATION           "Jste si jistý?"
    #define CZ_DOWN                   "Dolů"
    #define CZ_UP                     "Nahoru"
    #define CZ_SAVE                   "Uložit"
    #define CZ_RESTORE                "Restore"
    #define CZ_RESET                  "Restart"
    #define CZ_DEFAULT                "Default"
    #define CZ_NEXT                   "Next"
    #define CZ_DISTANCE               "Distance"
    #define CZ_INVALID_VALUE          "No valid value/s provided!"
    #define CZ_TIMEOUT_REACHED        "Timeout reached!"
    #define CZ_PROCESS_RUNNING        "Process running! Wait completion"
    #define CZ_PROCESS_COMPLETED      "Process completed!"
    #define CZ_PROCESS_ABORTED        "Process aborted!"
    #define CZ_APPLY                  "Použít"
    #define CZ_INC                    "Přidat"
    #define CZ_DEC                    "Ubrat"
    #define CZ_NOZZLE                 "Tryska"
    #define CZ_BED                    "Stolek"
    #define CZ_CHAMBER                "Komora"
    #define CZ_START                  "Start"
    #define CZ_STOP                   "Stop"
    #define CZ_BACK                   "Zpět"
    #define CZ_PAGE_UP                "Další"
    #define CZ_PAGE_DOWN              "Předchozí"
    #define CZ_PAUSE                  "Pauza"
    #define CZ_RESUME                 "Obnovit"
    #define CZ_LOAD                   "Zavést"
    #define CZ_UNLOAD                 "Vysunout"
    #define CZ_SLOW_SPEED             "Pomalu"
    #define CZ_NORMAL_SPEED           "Normál"
    #define CZ_FAST_SPEED             "Rychle"
    #define CZ_FAN_FULL_SPEED         "Plný"
    #define CZ_FAN_HALF_SPEED         "Poloviční"
    #define CZ_ROTATE_UI              "Otočit UI"
    #define CZ_TOUCHSCREEN_ADJUST     "Kalibrace"
    #define CZ_MORE                   "Více"
    #define CZ_SCREEN_INFO            "Info"
    #define CZ_STATUS                 "Status"
    #define CZ_SIMULATOR_BG_COLOR     "Marlin simulator. Barva pozadí"
    #define CZ_SIMULATOR_FONT_COLOR   "Marlin simulator. Barva písma"
    #define CZ_WHITE                  "Bílá"
    #define CZ_BLACK                  "Černá"
    #define CZ_BLUE                   "Modrá"
    #define CZ_RED                    "Červená"
    #define CZ_GREEN                  "Zelená"
    #define CZ_CYAN                   "Tyrkys"
    #define CZ_YELLOW                 "Žlutá"
    #define CZ_BROWN                  "Hnědá"
    #define CZ_GRAY                   "Šedá"
    #define CZ_ORANGE                 "Oranžová"
    #define CZ_INDIGO                 "Indigo"
    #define CZ_VIOLET                 "Fialová"
    #define CZ_MAGENTA                "Purpurová"
    #define CZ_PURPLE                 "Nachová"
    #define CZ_LIME                   "Limetková"
    #define CZ_DARKBLUE               "Tmavá Modrá"
    #define CZ_DARKGREEN              "Tmavá Červená"
    #define CZ_DARKGRAY               "Tmavá Šedá"
    #define CZ_DISCONNECT             "Odpojit"
    #define CZ_BAUDRATE               "Serial Port"
    #define CZ_PERCENTAGE             "Procenta"
    #define CZ_BABYSTEP               "BabyStep"
    #define CZ_PERCENTAGE_SPEED       "Rychlost"
    #define CZ_PERCENTAGE_FLOW        "Průtok"
    #define CZ_VALUE_ZERO             "Vynulovat"
    #define CZ_1_DEGREE               "1℃"
    #define CZ_5_DEGREE               "5℃"
    #define CZ_10_DEGREE              "10℃"
    #define CZ_X_INC                  "X+"
    #define CZ_Y_INC                  "Y+"
    #define CZ_Z_INC                  "Z+"
    #define CZ_X_DEC                  "X-"
    #define CZ_Y_DEC                  "Y-"
    #define CZ_Z_DEC                  "Z-"
    #define CZ_X                      "X"
    #define CZ_Y                      "Y"
    #define CZ_Z                      "Z"
    #define CZ_001_MM                 "0,01mm"
    #define CZ_01_MM                  "0,1mm"
    #define CZ_1_MM                   "1mm"
    #define CZ_5_MM                   "5mm"
    #define CZ_10_MM                  "10mm"
    #define CZ_100_MM                 "100mm"
    #define CZ_200_MM                 "200mm"
    #define CZ_1_PERCENT              "1%"
    #define CZ_5_PERCENT              "5%"
    #define CZ_10_PERCENT             "10%"
    #define CZ_PERCENT_VALUE          "%d%%"
    #define CZ_READY                  "Připraven"
    #define CZ_BUSY                   "Zpracovávám, čekejte prosím..."
    #define CZ_UNCONNECTED            "Tiskárna není připojena!"
    #define CZ_DISCONNECT_INFO        "Nyní můžete tiskárnu ovládat z počítače!"
    #define CZ_LOADING                "Nahrávám..."
    #define CZ_POWER_FAILED           "Obnovit tisk?"
    #define CZ_CONTINUE               "Pokračovat"
    #define CZ_CANCEL                 "Zrušit"
    #define CZ_ADJUST_TITLE           "Kalibrace dotykové obrazovky"
    #define CZ_ADJUST_INFO            "Dotkněte se postupně všech bodů"
    #define CZ_ADJUST_OK              "Kalibrace úspěšná"
    #define CZ_ADJUST_FAILED          "Kalibrace selhala, zopakujte znovu"
    #define CZ_WARNING                "Varování"
    #define CZ_STOP_PRINT             "Zastavit tisk?"
    #define CZ_CONFIRM                "Potvrdit"
    #define CZ_TFTSD                  "TFT SD"
    #define CZ_READ_TFTSD_ERROR       "Chyba při čtení SD karty!"
    #define CZ_TFTSD_INSERTED         "SD karta vložena!"
    #define CZ_TFTSD_REMOVED          "SD karta vyjmuta!"
    #define CZ_U_DISK                 "USB Disk"
    #define CZ_READ_U_DISK_ERROR      "Chyba při čtení USB disku!"
    #define CZ_U_DISK_INSERTED        "USB disk  vložen!"
    #define CZ_U_DISK_REMOVED         "USB disk vyjmut!"
    #define CZ_ONBOARDSD              "Onboard SD"
    #define CZ_READ_ONBOARDSD_ERROR   "Chyba při čtení Onboard SD!"
    #define CZ_FILAMENT_SENSOR        "Filament sensor"
    #define CZ_FILAMENT_RUNOUT        "Došel filament!"
    #define CZ_PREHEAT                "Zahřát"
    #define CZ_PREHEAT_BOTH           "Vše"
    #define CZ_IS_PAUSE               "Během tisku nelze\nextrudovat,\nmám pozastavit tisk?"
    #define CZ_AUTO_SHUT_DOWN         "Automat"
    #define CZ_UNIFIEDMOVE            "Pohyb"
    #define CZ_UNIFIEDHEAT            "Teplota"
    #define CZ_COOLDOWN               "Zchladit"
    #define CZ_EMERGENCYSTOP          "!!STOP!!"
    #define CZ_TOUCH_TO_EXIT          "Pro návrat se dotkněte obrazovky..."
    #define CZ_MAINMENU               "Menu"
    #define CZ_WAIT_TEMP_SHUT_DOWN    "Čekám na zchladnutí\nextruderu pod %d℃" // Wait for the temperature of hotend to be lower than 50℃
    #define CZ_FORCE_SHUT_DOWN        "Vynutit"
    #define CZ_SHUTTING_DOWN          "Vypínám se..."
    #define CZ_PARAMETER_SETTING      "Parametr"
    #define CZ_ON                     "ON"
    #define CZ_OFF                    "OFF"
    #define CZ_SMART                  "SMART"
    #define CZ_TERMINAL_ACK           "zobrazit ACK v Terminal"
    #define CZ_INVERT_XAXIS           "Otočit osu X"
    #define CZ_INVERT_YAXIS           "Otočit osu Y"
    #define CZ_INVERT_ZAXIS           "Otočit osu Z"
    #define CZ_MOVE_SPEED             "Rychlost pohybu X Y Z"
    #define CZ_KNOB_LED               "Rotační knoflík LED"
    #define CZ_KNOB_LED_IDLE          "Rotary Knob led idle"
    #define CZ_M0_PAUSE               "Pauza po příkazu M0"
    #define CZ_SEND_START_GCODE       "Spustit Gcode před tiskem"
    #define CZ_SEND_END_GCODE         "Spustit Gcode po tisku"
    #define CZ_SEND_CANCEL_GCODE      "Přerušení tisku"
    #define CZ_PERSISTENT_STATUS_INFO "Trvalé zobrazení stavu"
    #define CZ_FILE_LISTMODE          "Zobrazit soubory jako seznam"
    #define CZ_CURRENT_SETTING        "Proud motorů (mA)"
    #define CZ_STEPS_SETTING          "Kroků/mm"
    #define CZ_MAXFEEDRATE            "Max rychlost"
    #define CZ_MAXACCELERATION        "Max zrychlení"
    #define CZ_ACCELERATION           "Zrychlení"
    #define CZ_PRINT_ACCELERATION     "Zrychlení při tisku"
    #define CZ_RETRACT_ACCELERATION   "Zrychlení retrakcí"
    #define CZ_TRAVEL_ACCELERATION    "Zrychlení při přesunu"
    #define CZ_BUMP_SENSITIVITY       "Citlivost TMC na náraz"
    #define CZ_FWRETRACT              "FW Zatažení"
    #define CZ_FWRECOVER              "FW Obnovení"
    #define CZ_LIN_ADVANCE            "Linear Advance"
    #define CZ_RESET_SETTINGS_INFO    "Všechna nastavení budou\nobnovena na výchozí\nhodnoty. Pokračovat?"
    #define CZ_RESET_SETTINGS_DONE    "Resetování všech nastavení\nbylo úspěšně provedeno.\nRestartujte zařízení."
    #define CZ_INFO                   "Info"
    #define CZ_LCD_BRIGHTNESS         "LCD Jas"
    #define CZ_LCD_BRIGHTNESS_DIM     "LCD snížení jasu"
    #define CZ_LCD_DIM_IDLE_TIMER     "LCD snížení jasu po"
    #define CZ_5_SECONDS              "5 sec."
    #define CZ_10_SECONDS             "10 sec."
    #define CZ_30_SECONDS             "30 sec."
    #define CZ_60_SECONDS             "1 min."
    #define CZ_120_SECONDS            "2 min."
    #define CZ_300_SECONDS            "5 min."
    #define CZ_CUSTOM_SECONDS         "Vlastní"
    #define CZ_ST7920_FULLSCREEN      "Celoobrazovkový režim Marlin"
    #define CZ_PLR_EN                 "Obnova po ztrátě napájení"
    #define CZ_SETTING_RESET          "Reset nastavení"
    #define CZ_SETTING_RESTORE        "Obnovení nastavení"
    #define CZ_EEPROM_SAVE_INFO       "Uložit nastavení tiskárny\ndo EEPROM?"
    #define CZ_EEPROM_RESTORE_INFO    "Obnovit nastavení z\nEEPROM?"
    #define CZ_EEPROM_RESET_INFO      "Reset EEPROM to stock printer settings?"
    #define CZ_RETRACT_FEEDRATE       "Rychlost zatažení"
    #define CZ_RETRACT_LENGTH         "Délka zatažení"
    #define CZ_RETRACT_SWAP_LENGTH    "Délka zatažení Swap"
    #define CZ_RETRACT_Z_LIFT         "Z zvýšení při zatažení"
    #define CZ_RECOVER_FEEDRATE       "Rychlost obnovení"
    #define CZ_SWAP_RECOVER_FEEDRATE  "Rychlost obnovení Swap"
    #define CZ_RECOVER_LENGTH         "Délka obnovení"
    #define CZ_SWAP_RECOVER_LENGTH    "Délka obnovení Swap"
    #define CZ_START_PRINT            "Spustit tisk:\n %s?"
    #define CZ_ACK_POPUP_TYPE         "ACK popup type"
    #define CZ_ACK_BUZZER             "ACK buzzer"
    #define CZ_LEVELING_EDGE_DISTANCE "Bed edge distance"
    #define CZ_XY_UNLOCK              "Odemkněte XY"
    #define CZ_TUNING                 "Tuning"
    #define CZ_PID                    "PID"
    #define CZ_PID_TITLE              "PID autotune"
    #define CZ_PID_START_INFO         "PID autotune will be started. It can take minutes. Continue?"
    #define CZ_PID_START_INFO_2       "PID autotune in progress!"
    #define CZ_PID_START_INFO_3       "DO NOT touch the screen until completed (green LED on)!"
    #define CZ_EXTRUDE_100            "Ext. 100mm"
    #define CZ_CONNECTION_SETTINGS    "Connection"
    #define CZ_ZERO_X                 "Zero X"
    #define CZ_ZERO_Y                 "Zero Y"
    #define CZ_ZERO_Z                 "Zero Z"
    #define CZ_HOME_X                 "Home X"
    #define CZ_HOME_Y                 "Home Y"
    #define CZ_HOME_Z                 "Home Z"
    #define CZ_HOME_XYZ               "Home XYZ"
    #define CZ_CUT                    "Cut"
#endif
